...
#' @param destination Destination path where to store the downloaded file
#' @param ... Ignored
...
dse_s3_download <- function(
  ...
  bucket <- stringr::str_extract(uri, "(?<=^[s|S]3:\\/\\/)(.*?)(?=\\/)")
  key <- stringr::str_extract(uri, sprintf("(?<=^[s|S]3:\\/\\/%s\\/)(.*?)$", bucket))
  ds3 <- dse_s3(s3_key = s3_key, s3_secret = s3_secret)
  ds3$download_file(bucket, key, file.path(destination, basename(uri)))
  return(invisible())
}